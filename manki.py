...
# Copyright (C) 2012  H. İbrahim Güngör
#
...
def convertImage(source, target, device, flags):
    ...
    try:
        ...
        print('Cannot read image file %s' % source)
        raise
    ...
    if flags & ImageFlags.Frame:
        image = frameImage(image, tuple(palette[:3]),
                           tuple(palette[-3:]), size)
    ...
    try:
        ...
        print('Cannot write image file %s' % target)
        raise


def printUsage():
    print "Run the script in a directory where the image files are laid."
...
if __name__ == "__main__":
    ...
    DefaultImageFlags = ImageFlags.Orient | \
                        ImageFlags.Resize | \
                        ImageFlags.Quantize
    ...
    imageFiles = []
    for i in imageTypes:
        imageFiles.extend(sorted(glob.glob(i)))

    newImagePath = os.path.basename(os.getcwd())

    if not os.path.exists(newImagePath):
        os.makedirs(newImagePath)

    with zipfile.ZipFile(newImagePath + ".cbz", "w",
                         compression=zipfile.ZIP_DEFLATED) as myzip:
        for imageFile in imageFiles:
            newFilename = os.path.splitext(imageFile)[0] + ".png"

            convertImage(imageFile, os.path.join(newImagePath, newFilename),
                    str(DefaultDevice), int(DefaultImageFlags))

            try:
                myzip.write(os.path.join(newImagePath, newFilename))
            except OSError:
                continue

    myzip.close()

    shutil.rmtree(newImagePath)
